...
export default function PageHero({ eyebrow, title, text, image, primary, secondary, compact = false }) {
  return (
    <section className={`relative isolate overflow-hidden ${compact ? "min-h-[420px]" : "min-h-[680px]"}`}>
      ...
      <div className={`container-page flex ${compact ? "min-h-[420px] pt-[calc(var(--site-header-height)+4rem)] lg:pt-[calc(var(--site-header-height)+5rem)]" : "min-h-[680px] pt-36"} items-center pb-16`}>
        <div className="max-w-3xl text-white">
          {eyebrow && <p className="mb-5 text-xs font-extrabold uppercase tracking-[0.24em] text-brass">{eyebrow}</p>}
